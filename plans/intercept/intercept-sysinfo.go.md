...
Replaces the switch cases in processinfo.go. Each command is its own type.
...
// NewSysInfoInterceptors returns all process-identity interceptors.
...
func (p *pwdInterceptor) Handle(ctx context.Context, args []string) error {
...
func (h *hostnameInterceptor) Handle(ctx context.Context, args []string) error {
	hc := interp.HandlerCtx(ctx)
	// hostname [-s] → short name (strip domain)
	// hostname [-f] → FQDN (we just return hostname as-is)
	// hostname [-i] → IP (return 127.0.0.1)
...
func (i *idInterceptor) Handle(ctx context.Context, args []string) error {
	hc := interp.HandlerCtx(ctx)
	cfg := i.cfg
	// Parse flags: -u (uid only), -g (gid only), -n (name not number), -G (all groups)
	numericOnly := true
	for _, a := range args[1:] {
		switch a {
		case "-u", "--user":
...
// Kernel/OS constants the sandbox pretends to be.
const (
	unameKernel   = "Linux"
...
func (u *unameInterceptor) Handle(ctx context.Context, args []string) error {
	...
	// Combine all flags (handles "-srm" same as "-s -r -m")
	combined := strings.Join(args[1:], "")
	if strings.ContainsAny(combined, "a") {
